...
namespace EmailSkill.Dialogs
{
    public class EmailSkillDialogBase : ComponentDialog
    {
        ...
        protected async Task<DialogTurnResult> GetMyNameAsync(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            var state = await EmailStateAccessor.GetAsync(sc.Context);
            var token = state.Token;
            var service = ServiceManager.InitUserService(token, state.GetUserTimeZone(), state.MailSourceType);
                ...
                var user = await service.GetMeAsync();
                    ...
                    string dane =  $"Hi there! {user.DisplayName}";
            await sc.Context.SendActivityAsync(dane);
            return await sc.NextAsync();
        ...
        protected async Task HandleDialogExceptions(WaterfallStepContext sc, Exception ex)
        {
            ...
            await sc.Context.SendActivityAsync(ResponseManager.GetResponse(ex.Message));

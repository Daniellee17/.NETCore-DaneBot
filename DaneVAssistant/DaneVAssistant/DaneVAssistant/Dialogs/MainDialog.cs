...
using DaneVAssistant.Responses.Main;
using DaneVAssistant.Services;
...
namespace DaneVAssistant.Dialogs
{
    public class MainDialog : RouterDialog
    {
        ...
        private IStatePropertyAccessor<UserProfile> _userProfile;
            ...
            : base(nameof(MainDialog), telemetryClient)
        {
            ...
            _userProfile = userState.CreateProperty<UserProfile>(nameof(UserProfile));
        ...
        protected override async Task OnStartAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userProfile = await _userProfile.GetAsync(dc.Context, () => new UserProfile());
        ...
        protected override async Task OnEventAsync(DialogContext dc, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (value.GetType() == typeof(JObject))
            {
                ...
                if (value != null && (string)submit["action"] == "getStarted")
                {
                    await OnHelp(dc);
                    return;
                }

                else if (value != null && (string)submit["action"] == "logout")
                {
                    userProfile.ClearAll();
                    await _userProfile.SetAsync(dc.Context, userProfile, cancellationToken);
                    await OnLogout(dc);
                    return;
                }
            }
                ...
                else
                {
                    ...
                    if (luisResult.TopIntent().score > 0.5)
                    {
                        switch (intent)
                        {
                            ...
                            case GeneralLuis.Intent.GoBack:
                                {
